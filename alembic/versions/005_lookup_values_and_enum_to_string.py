...
def upgrade() -> None:
    ...
    # PostgreSQL requires explicit ALTER COLUMN TYPE with USING clause.
    # SQLite stores enums as strings already, so we use dialect-specific logic.
    bind = op.get_bind()
    dialect = bind.dialect.name

    if dialect == 'postgresql':
        # PostgreSQL: ALTER COLUMN TYPE with USING to cast existing values
        ...
        # department_balances.metal_type: also uses Enum(MetalType) -> String(50)
        op.execute(
        ...
        # SQLite: enum columns are already stored as VARCHAR, no conversion needed.
        # This branch handles testing with SQLite in-memory databases.
        pass
